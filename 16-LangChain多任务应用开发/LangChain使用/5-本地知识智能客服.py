...
# 定义一个自定义的提示模板类 CustomPromptTemplate，继承自 StringPromptTemplate。
# 该类的主要作用是根据给定的模板和工具，结合中间步骤的执行结果，生成填充好的提示文本。
# 在多步骤的 Agent 推理过程中，它会将每一步的思考、行动和观察结果整合到提示模板中，
# 帮助语言模型了解当前的推理进度，从而做出下一步的决策。
class CustomPromptTemplate(StringPromptTemplate):
    ...
    def format(self, **kwargs) -> str:
        ...
        # 枚举所有的工具名称
        # 枚举所有的工具名称，将每个工具的名称提取出来，使用逗号加空格连接成一个字符串，
        # 并将结果存储在 kwargs 字典的 "tool_names" 键中。
        # 这个字符串会在模板中使用，让 Agent 知道可使用的工具名称有哪些。
...
class CustomOutputParser(AgentOutputParser):
    def parse(self, llm_output: str) -> Union[AgentAction, AgentFinish]:
        ...
        # 使用正则表达式 regex 在 llm_output 中进行搜索，re.DOTALL 标志表示 '.' 可以匹配包括换行符在内的所有字符。
        # 若匹配成功，match 将包含匹配对象；若未匹配到，match 将为 None。
        # 这里的 regex 用于解析出 LLM 输出中的 Action 和 Action Input 部分。
...
if __name__ == "__main__":
    ...
    # 用户定义的模板
    # 创建一个自定义的提示模板实例，用于生成Agent执行时所需的提示文本
    # CustomPromptTemplate 类继承自 StringPromptTemplate，能够根据模板和工具信息，结合中间步骤结果生成填充后的提示
    # template=AGENT_TMPL：指定使用预定义的 AGENT_TMPL 作为基础模板，该模板定义了Agent回答问题的格式和规则
    # tools=tools：传入可使用的工具列表，这些工具会在模板中被展示，供Agent选择使用
    # input_variables=["input", "intermediate_steps"]：指定模板需要的输入变量，
    # "input" 代表用户输入的问题，"intermediate_steps" 代表Agent推理过程中的中间步骤
