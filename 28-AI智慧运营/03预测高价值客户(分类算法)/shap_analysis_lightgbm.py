"""
使用LightGBM和SHAP(可解释模型)分析预测客户未来3个月资产是否能提升至100万+
SHAP包含全局解释和局部解释，全局解释展示了每个特征对模型预测的整体影响，局部解释展示了单个样本的预测结果和特征贡献。
SHAP可以使用于树模型解释器（TreeExplainer）或线性模型解释器（LinearExplainer）、神经网络解释器（DeepExplainer）等进行分析。
"""
import pandas as pd
import numpy as np
...
def shap_global_analysis(model, X, features):
    """SHAP全局解释分析"""
    print("\n=== SHAP全局解释分析 ===")
    ...
    if isinstance(shap_values, list):
        shap_values = shap_values[1]  # 二分类情况下取正类
    
    # 打印特征重要性（基于SHAP值的平均绝对值）
...
def shap_local_analysis(model, X, features, explainer, shap_values, sample_indices=None):
    """SHAP局部解释分析（针对单个客户）"""
    print("\n=== SHAP局部解释分析（针对单个客户）===")
    
    if sample_indices is None:
        # 随机选择几个客户进行局部解释
        sample_indices = np.random.choice(X.index, size=min(3, len(X)), replace=False)
    
    # 获取原始数据以显示客户信息
    df_original = load_and_preprocess_data()
    
    for idx in sample_indices:
        print(f"\n--- 客户 {df_original.loc[df_original.index == idx, 'customer_id'].iloc[0]} 的SHAP解释 ---")
        print(f"该客户当前总资产: {df_original.loc[df_original.index == idx, 'total_assets'].iloc[0]}")
        print(f"该客户月收入: {df_original.loc[df_original.index == idx, 'monthly_income'].iloc[0]}")
        
        # 获取该样本的索引位置
        X_idx = X.index.get_loc(idx)
        
        # 获取该客户的SHAP值
        shap_value = shap_values[X_idx, :]
        shap_value_for_sample = shap_value
        
        # 创建DataFrame以查看每个特征的贡献
        feature_contributions = pd.DataFrame({
            'Feature': features,
            'Value': X.loc[idx, :].values,
            'SHAP_Value': shap_value_for_sample
        }).sort_values(by='SHAP_Value', key=abs, ascending=False)  # 按绝对值排序
        ...
        plt.tight_layout()
        plt.savefig(f'shap_waterfall_customer_{idx}.png', dpi=300, bbox_inches='tight')
        plt.show()
...
if __name__ == '__main__':
    print('===== 使用LightGBM和SHAP预测客户未来3个月资产是否能提升至100万+ =====')
    ...
    # 5. SHAP全局解释分析
    print('\n5. SHAP全局解释分析...')
    shap_values, explainer = shap_global_analysis(model, X, features)
    
    # 6. SHAP局部解释分析
    print('\n6. SHAP局部解释分析...')
    shap_local_analysis(model, X, features, explainer, shap_values)
    ...
    print('SHAP全局解释图已保存为 shap_summary_plot.png 和 shap_bar_plot.png')
    print('SHAP局部解释图已保存为 shap_waterfall_customer_*.png')