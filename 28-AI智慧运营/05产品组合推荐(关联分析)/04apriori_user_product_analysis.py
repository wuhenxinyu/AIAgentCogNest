"""
不仅可以放 产品，也可以放用户画像，这样更好的挖掘不同用户画像，喜欢购买哪些产品，以及适合推荐哪些产品lift更高,编写到 新的 .py
...
def main():
    ...
    print(f"\n找到 {len(rules)} 条关联规则")
    print("\n前20条关联规则 (按置信度降序, 仅显示提升度>1的规则):")

    # 过滤出提升度大于1的规则，即关联性强的规则
    high_lift_rules = [rule for rule in rules if rule['lift'] > 1]
    sorted_rules = sorted(high_lift_rules, key=lambda x: x['confidence'], reverse=True)
    ...
    highest_lift_rules = sorted(high_lift_rules, key=lambda x: x['lift'], reverse=True)
    for i, rule in enumerate(highest_lift_rules[:10]):
        print(f"{set(rule['antecedent'])} -> {set(rule['consequent'])}")
        print(f"  支持度: {rule['support']:.4f}, 置信度: {rule['confidence']:.4f}, 提升度: {rule['lift']:.4f}")
        print()
