...
try:
    ...
    clip_model = CLIPModel.from_pretrained("/dev/shm/models/openai/clip-vit-base-patch32", local_files_only=True).to(device)
...
def get_image_embedding(image_path):
    ...
    # padding=True: 对批处理中的图像进行填充,使它们具有相同的尺寸
    ...
    # 4. 正确的设备检查（只检查pixel_values）
    assert "pixel_values" in inputs, "处理器未返回pixel_values！"
    assert inputs["pixel_values"].device == device, "输入图像与模型设备不一致！"
    ...
    # 立即转移结果到CPU并释放GPU资源
    results = image_features.cpu().numpy() # 返回CPU上的numpy数组
    print(f"get_image_embedding推理后显存: {torch.cuda.memory_allocated() / 1024**2:.2f} MB")
    del image,inputs, image_features
    torch.cuda.empty_cache()
    print(f"get_image_embedding清理后显存: {torch.cuda.memory_allocated() / 1024**2:.2f} MB")    
...
def get_clip_text_embedding(text):
    ...
    print(f"get_clip_text_embedding推理后显存: {torch.cuda.memory_allocated() / 1024**2:.2f} MB")
    # 立即转移结果到CPU并释放GPU资源
    results = text_features.cpu().numpy() # 返回CPU上的numpy数组
    del inputs, text_features
    torch.cuda.empty_cache()
    print(f"get_clip_text_embedding清理后显存: {torch.cuda.memory_allocated() / 1024**2:.2f} MB")
    return results
