...
class Memory(Agent):
    ...
    def __init__(self,
        ...
        if self.rag_backend == 'elasticsearch':
            # 如果配置为 elasticsearch，则加载我们自定义的 ES 检索工具（BM25搜索）
            print("[Memory] 使用 Elasticsearch 后端进行检索 (BM25)。")
        ...
        elif self.rag_backend == 'elasticsearch_embedding':
            # 如果配置为 elasticsearch_embedding，则加载自定义的 ES Embedding 检索工具
            print("[Memory] 使用 Elasticsearch + Embedding 后端进行检索 (向量搜索)。")
            retrieval_tool = ESRetrievalTool(cfg=self.cfg)
            # 将其实例添加到 function_list 中，它会因为同名'retrieval'而自动覆盖
            function_list.append(retrieval_tool)
        else:
