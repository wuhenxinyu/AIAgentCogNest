...
class DocParser(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        # 修改：使用 pop 获取 use_cache 参数，以避免重复传递
        ...
        try:
            ...
            return json.dumps({'error': f'Invalid JSON input: {params}'})

        if 'url' not in params:
            return json.dumps({'error': 'the `url` parameter is required.'})

        url = params['url']
        # 修改：将 use_cache 和 kwargs 传递给 _parse_doc
        return json.dumps(self._parse_doc(url, use_cache=use_cache, **kwargs), ensure_ascii=False)

    def _parse_doc(self, url: str, use_cache: bool, **kwargs) -> list:
        """Parse one file in the user-provided file list.
        ...
        Returns:
            A list of dicts, each dict represents a chunk of the file.
        """
        ...
        if use_cache:
            cached_file_path = os.path.join(self.data_root, cached_name_chunking)
            if os.path.isfile(cached_file_path):
                self.logger.info(f'Read chunked {url} from cache.')
                with open(cached_file_path, 'r', encoding='utf-8') as f:
                    return json.loads(f.read())
        
        # If not using cache or not found in cache, parse the document
        ...
        self.logger.info(f'Start chunking {url} ({title})...')
        time1 = time.time()
        ...
        time2 = time.time()
        self.logger.info(f'Finished chunking {url} ({title}). Time spent: {time2 - time1} seconds.')
